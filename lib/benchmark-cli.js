...
var benchmarks = {}
var benchmarkDir = path.resolve(__dirname, '..', 'benchmark')
fs.readdirSync(benchmarkDir)
  .filter(function (name) {
    return fs.statSync(path.resolve(benchmarkDir, name)).isDirectory()
  })
  .forEach(function (category) {
    benchmarks[category] = fs.readdirSync(path.resolve(benchmarkDir, category))
...
CLI.prototype.benchmarks = function () {
  ...
  this.items.forEach((category) => {
    if (benchmarks[category] === undefined) { return }

    benchmarks[category].forEach((scripts) => {
      if (filter && scripts.lastIndexOf(filter) === -1) { return }
